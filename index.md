...
Same plot. One needs `plotly`, `ggrepel`, and a paragraph of `theme()`
calls to get interactive, styled, and readable. The other needs a pipe.

[TABLE]

No `aes()`. No [`factor()`](https://rdrr.io/r/base/factor.html)
coercion. No `+` juggling geometry, scales, and theme in one flat
expression. And you get zoom, brush, and hover for free.
